...
const createWithdrawal = async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query(
            "INSERT INTO withdrawals (network, address, amount, user_id, user_id_fk) VALUES (?, ?, ?, ?,?)",
            [network, address, amount, userId, username]
        );
...
const getWithdrawalHistory = async (req, res) => {
    ...
    try {
        ...
        const [withdrawals] = await pool.query(
            "SELECT address, amount, user_id_fk, created_at FROM withdrawals WHERE user_id = ? ORDER BY created_at DESC",
            [userId]
        );
